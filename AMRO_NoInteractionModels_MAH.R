...
AMRO <- AMRO[complete.cases(AMRO[,c('FATE','EXP','Year','TrailDist', 'WTR', 'WBH', 'GPH',
                                    'RoadDist', 'EdgeDist')]),]
...
#Center and scale
head(AMRO.MAH[,c(13:25)])
str(AMRO.MAH)
for (i in 13:25){AMRO.MAH[,i] <- (AMRO.MAH[,i] - mean(AMRO.MAH[,i]))/sd(AMRO.MAH[,i])}
...
####################################
str(AMRO.MAH)
AMRO.MAH.BMA <- bic.glm(FATE ~ Core10km + Edge60m + OrdDate + CoreArea + TrailDist+ WBH + GPH + EdgeDist +PatchType + PlotType, glm.family = binomial(logexp(exposure = AMRO.MAH$EXP)), data = AMRO.MAH)
