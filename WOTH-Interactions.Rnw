...
\title{Follow-up analysis Wood Thrush (\textsc{WOTH}) nest success along trails\\ (with interactions)}
...
source("WOTH_InteractionModels_MJP.R")
source("WOTH_InteractionModels_MAH.R")
source("WOTH_InteractionModels_Combined.R")
...
%' 
%' @
%' \caption{Correlation of potential explanatory variables for \textsc{MJP WOTH} nests}
%' \label{fig:MJPCorr}
%' \end{figure}
%' \begin{center}
%' \end{center}
...
%' @
%' \caption{Correlation of potential explanatory variables for \textsc{MAH WOTH} nests}
%' \label{fig:MAHCorr}
%' \end{figure}
%' \begin{center}
%' \end{center}
...
\begin{figure}[!htbp]
<<echo=FALSE, results='hide'>>=
imageplot.bma(WOTH.BMA, color = c("blue", "red", "white"), order = "mds")
@
